...
data = pd.read_csv(
    "/content/drive/MyDrive/Colab Notebooks/deploy-ml-web-workshop/mbpt_dataset.csv")
...
print('Train Classification Report \n ', classification_report(train_target, model_svc.predict(
    train_post), zero_division=0, target_names=target_encoder.inverse_transform([i for i in range(16)])))

print('Test Classification Report \n ', classification_report(test_target, model_svc.predict(
    test_post), zero_division=0, target_names=target_encoder.inverse_transform([i for i in range(16)])))

models_accuracy['Support Vector Classifier'] = accuracy_score(
    test_target, model_svc.predict(test_post))
...
print('Train Classification Report \n ', classification_report(train_target, model_multinomial_nb.predict(
    train_post), zero_division=0, target_names=target_encoder.inverse_transform([i for i in range(16)])))

print('Test Classification Report \n ', classification_report(test_target, model_multinomial_nb.predict(
    test_post), zero_division=0, target_names=target_encoder.inverse_transform([i for i in range(16)])))

models_accuracy['Multinomial Naive Bayes'] = accuracy_score(
    test_target, model_multinomial_nb.predict(test_post))
...
print('Train Classification Report \n ', classification_report(train_target, model_forest.predict(
    train_post), zero_division=0, target_names=target_encoder.inverse_transform([i for i in range(16)])))

print('Test Classification Report \n ', classification_report(test_target, model_forest.predict(
    test_post), zero_division=0, target_names=target_encoder.inverse_transform([i for i in range(16)])))

models_accuracy['Random Forest Classifier'] = accuracy_score(
    test_target, model_forest.predict(test_post))

...
3. In the `app.py` file, add the code below (same as the last lesson, but we will make some changes for the form requests):
...
4. In the `base.html` file, add the code below:
...
5. In the `main.html` file, add the code below:
...
6. In `main.js`, add the code below to populate the country select field with the list of countries in the [`countries.json`](../app/flask-app/static/countries.json) file:
...
6. In `result.html`, add the code below to display the result from the API:
...
7. In `404.html`, add the code below to display the 404 error page:
...
8. Now, use the command `flask run` to start the Flask server on `http://127.0.0.1:5000`.
